-- NOCT 0018: taste declared at onboarding.
--
-- Recommendations needed a going/saved history, so a new account saw nothing until it had marked something --
-- the classic cold start. Onboarding asks two skippable questions instead: a few genres, then a few real
-- nights. The nights become ordinary `saved` rows and need no special case; the genres live here.
--
-- Weighted 0.7 against a going's 1.0 and a saved's 0.4: a stated preference is a real signal but a weaker one
-- than turning up, and it must never outrank what someone actually did.
set search_path = public, extensions;

alter table profile add column if not exists taste_genres text[] not null default '{}';
alter table profile add column if not exists onboarded_at timestamptz;
comment on column profile.taste_genres is 'Genre codes picked at onboarding; seeds recommendations and the feed sort before there is any history.';
