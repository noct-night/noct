-- NOCT 0021: the line-up the model read, kept where ingest cannot overwrite it.
--
-- 684 of 1,531 upcoming events have no line-up: RA and 19hz both title the night "Ian Asher" and leave the
-- field empty. A title heuristic gets about three in four, which is fine for a tap target and not fine for a
-- record; deciding whether a title is a person or a party is a judgement, so the classifier does it (p2).
--
-- It cannot live in event.lineup: refresh_event() rebuilds that from listing.lineup_raw on every ingest and
-- would wipe it. event_feed reads the source line-up first and falls back to this one.
--
-- Deliberately NOT wired into event_artist yet. Artists drive recommendations, and a model reading is worth
-- showing before it is worth treating as a fact; that comes once the quality has been measured.
set search_path = public, extensions;

alter table event add column if not exists lineup_model text[] not null default '{}';
comment on column event.lineup_model is
  'Performers the classifier read out of the evidence when no source supplied a line-up. Display only; refresh_event() never touches it.';
