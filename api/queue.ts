/**
 * GET /queue — the review page, assembled and served by a function rather than as a static file.
 *
 ...
 * -- not the queue markup, not the endpoint names, not the shape of the API.
 *
 * The source is three files under queue/ for the same reason the app is index.html + app.css + app.js: UI
...
/**
 * Where queue/ ends up depends on how the function was built, so try the candidates rather than assume.
 * vercel.json's `includeFiles` puts the directory in the bundle; locally the repo root is the cwd.
 */
function candidates(): string[] {
  const here = dirname(fileURLToPath(import.meta.url));
  return [join(process.cwd(), 'queue'), join(here, '..', 'queue'), join(here, 'queue')];
}
...
async function readAsset(name: string): Promise<string> {
  ...
  throw new Error(`could not find queue/${name}; looked in ${tried.join(', ')}`);
}
 ...
 *
 * Signed out gets the door and gate.js and nothing else: no queue markup, and no page.js, which would
 * otherwise describe every endpoint and every action the queue can take. The endpoints are gated
...
async function buildPage(signedIn: boolean): Promise<string> {
  ...
  return signedIn
    ...
    : stripQueue(base).replace('/*noct:js*/', '');
}
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  try {
    // The door and the queue are one document, and the server decides which half exists. Without a session
    // the queue half is cut out of the response rather than hidden behind an attribute: markup that is
    // never sent is markup that cannot be read.
    ...
    log.error('queue page failed', { error });
    sendJson(res, 500, { error: 'queue page unavailable' }, { 'cache-control': 'no-store' });
  }
}

const QUEUE_START = '<!--noct:queue-start-->';
const QUEUE_END = '<!--noct:queue-end-->';

/**
 * Everything between the queue markers, removed, and the door revealed in its place.
 *
 ...
 */
export function stripQueue(html: string): string {
  const start = html.indexOf(QUEUE_START);
  const end = html.indexOf(QUEUE_END);
  if (start < 0 || end < 0) return html;
  return `${html.slice(0, start)}${html.slice(end + QUEUE_END.length)}`
